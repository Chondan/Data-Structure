...
void insertAtPosition(int value, int pos, List *list); // DONE
	...
	while (1) {
		...
		switch (choice) {
			...
			case 3:
				...
				scanf("%d", &pos);
				insertAtPosition(value, pos, list);
				break;
			case 4: 
				printf("\033[1;33m");
...
void insertAtEnd(int value, List *list) {
	...
	Node *oldLast = getNodePosition(getSize(list) - 1, list);
...
void removeAtBeginning(List *list) {
	...
	Node *firstNode = getNodePosition(1, list);
	Node *lastNode = getNodePosition(getSize(list) - 1, list);
	list->head = firstNode;
...
void removeAtEnd(List *list) {
	...
	Node *lastNode = getNodePosition(getSize(list) - 2, list);
